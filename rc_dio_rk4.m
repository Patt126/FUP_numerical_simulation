...
% Runge-Kutta 4
%
for t = 1:length(T)-1 % forse bisogna andare fino a length(T)-1 controllare prima di consegnare
    f1 = F(T(t),sol_pos(t,:)');
    ...
    g1 = A(T(t),sol_vel(t+1,:)');
    g2 = A(T(t)+dt/2,sol_vel(t+1,:)'+dt*g1/2);
    g3 = A(T(t)+dt/2,sol_vel(t+1,:)'+dt*g2/2);
    g4 = A(T(t+1),sol_vel(t+1,:)'+dt*g3);
    sol_pos(t+1,:) = sol_pos(t,:) + dt*(g1+2*g2+2*g3+g4)'/6;
end

solution = [sol_vel,sol_pos];
